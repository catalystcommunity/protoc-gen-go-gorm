...
func (s *PostgresPluginSuite) TestUpsertWithFunc() {
	// create profiles
	numProfiles := gofakeit.Number(2, 5)
	profiles := getPostgresProfiles(s.T(), numProfiles)
	var theName string
	models, err := Upsert[*Profile, *ProfileGormModel](context.Background(), postgresDb, profiles, func(ctx context.Context, tx *gorm.DB, protos []Proto[*ProfileGormModel], models []*ProfileGormModel) error {
...
func (s *PostgresPluginSuite) TestUpsertWithFuncError() {
	// create profiles
	numProfiles := gofakeit.Number(2, 5)
	profiles := getPostgresProfiles(s.T(), numProfiles)
	_, err := Upsert[*Profile, *ProfileGormModel](context.Background(), postgresDb, profiles, func(ctx context.Context, tx *gorm.DB, protos []Proto[*ProfileGormModel], models []*ProfileGormModel) error {
...
func (s *PostgresPluginSuite) TestDeleteWithFunc() {
	// create profiles
	numProfiles := gofakeit.Number(5, 10)
	profiles := getPostgresProfiles(s.T(), numProfiles)
	_, err := Upsert[*Profile, *ProfileGormModel](context.Background(), postgresDb, profiles, nil)
	require.NoError(s.T(), err)
	// delete some
	ids := lo.Map(profiles, func(item *Profile, index int) string { return *item.Id })
	idsToDelete := ids[:3]
	var weh string
	err = Delete[*ProfileGormModel](context.Background(), postgresDb, idsToDelete, func(ctx context.Context, tx *gorm.DB, ids []string, models []*ProfileGormModel) error {
		weh = "weh"
		return nil
	})
	require.NoError(s.T(), err)
	...
	// verify delete
	fetched, err := GetByIds[*ProfileGormModel](context.Background(), postgresDb, ids, nil)
	require.NoError(s.T(), err)
...
func (s *PostgresPluginSuite) TestDeleteWithFuncErr() {
	// create profiles
	numProfiles := gofakeit.Number(5, 10)
	profiles := getPostgresProfiles(s.T(), numProfiles)
	_, err := Upsert[*Profile, *ProfileGormModel](context.Background(), postgresDb, profiles, nil)
...
func (s *PostgresPluginSuite) TestPreloadBelongsTo() {
	...
	_, err := Upsert[*Company, *CompanyGormModel](context.Background(), postgresDb, []*Company{company}, nil)
	require.NoError(s.T(), err)
	user := getPostgresUser(s.T())
	user.CompanyId = company.Id
	_, err = Upsert[*User, *UserGormModel](context.Background(), postgresDb, []*User{user}, nil)
	require.NoError(s.T(), err)
	// get with preload
...
func (s *PostgresPluginSuite) TestPreloadHasOne() {
	...
	_, err = Upsert[*Address, *AddressGormModel](context.Background(), postgresDb, []*Address{address}, nil)
	require.NoError(s.T(), err)
	// get with preload
...
func (s *PostgresPluginSuite) TestPreloadHasMany() {
	...
	_, err = Upsert[*Comment, *CommentGormModel](context.Background(), postgresDb, comments, nil)
	require.NoError(s.T(), err)
	// get with preload
...
func (s *PostgresPluginSuite) TestPreloadManyToMany() {
	...
	user := getPostgresUser(s.T())
	userModels, err := Upsert[*User, *UserGormModel](context.Background(), postgresDb, []*User{user}, nil)
	require.NoError(s.T(), err)
	numProfiles := gofakeit.Number(2, 5)
	profiles := getPostgresProfiles(s.T(), numProfiles)
	profileModels, err := Upsert[*Profile, *ProfileGormModel](context.Background(), postgresDb, profiles, nil)
	require.NoError(s.T(), err)
	// associate the users and profiles
	associations := map[*UserGormModel][]*ProfileGormModel{
		userModels[0]: profileModels,
	}
	err = AssociateManyToMany[*UserGormModel, *ProfileGormModel](context.Background(), postgresDb, associations, "Profiles", nil)
	require.NoError(s.T(), err)
	// get with preload
...
func (s *PostgresPluginSuite) TestAssociateManyToManyWithFunc() {
	// create a user and profiles
	user := getPostgresUser(s.T())
	userModels, err := Upsert[*User, *UserGormModel](context.Background(), postgresDb, []*User{user}, nil)
	require.NoError(s.T(), err)
	...
	var weh string
	err = AssociateManyToMany[*UserGormModel, *ProfileGormModel](context.Background(), postgresDb, associations, "Profiles", func(ctx context.Context, tx *gorm.DB) error {
		weh = "weh"
		return nil
	})
	require.NoError(s.T(), err)
...
func (s *PostgresPluginSuite) TestAssociateManyToManyWithFuncErr() {
	// create a user and profiles
	user := getPostgresUser(s.T())
	userModels, err := Upsert[*User, *UserGormModel](context.Background(), postgresDb, []*User{user}, nil)
	require.NoError(s.T(), err)
...
func (s *PostgresPluginSuite) TestDissociateManyToMany() {
	...
	err = DissociateManyToMany[*UserGormModel, *ProfileGormModel](context.Background(), postgresDb, dissociations, "Profiles", nil)
	require.NoError(s.T(), err)
	// get with preload
...
func (s *PostgresPluginSuite) TestDissociateManyToManyWithFunc() {
	// create a user and profiles
	user := getPostgresUser(s.T())
	userModels, err := Upsert[*User, *UserGormModel](context.Background(), postgresDb, []*User{user}, nil)
	require.NoError(s.T(), err)
	...
	var weh string
	err = DissociateManyToMany[*UserGormModel, *ProfileGormModel](context.Background(), postgresDb, dissociations, "Profiles", func(ctx context.Context, tx *gorm.DB) error {
		weh = "weh"
		return nil
	})
	require.NoError(s.T(), err)
...
func (s *PostgresPluginSuite) TestDissociateManyToManyWithFuncErr() {
	// create a user and profiles
	user := getPostgresUser(s.T())
	userModels, err := Upsert[*User, *UserGormModel](context.Background(), postgresDb, []*User{user}, nil)
	require.NoError(s.T(), err)
...
func (s *PostgresPluginSuite) TestGetByIds() {
	...
	numProfiles := gofakeit.Number(5, 10)
	profiles := getPostgresProfiles(s.T(), numProfiles)
	upsertedProfiles, err := Upsert[*Profile, *ProfileGormModel](context.Background(), postgresDb, profiles, nil)
	require.NoError(s.T(), err)
	// get by id
